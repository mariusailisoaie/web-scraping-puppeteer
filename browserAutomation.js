...
(async () => {
  ...
  const docsNo = await page.evaluate(() => {
    document.querySelector('#panel-395-1-0-0 > div > ul > li:nth-child(4) > ul > li > ul').style.display = 'block';
    document.querySelector('#panel-395-1-0-0 > div > ul > li:nth-child(4) > ul > li > ul').style.display = 'block';
    return docNumber = Array.from(document.querySelectorAll('#panel-395-1-0-0 > div > ul > li:nth-child(4) > ul > li > ul')[0].children).map(li => li.innerHTML.split("'")[1]);
  });
  ...
  for (let i = 0; i < docsNo.length; i++) {
    ...
    let salary = await page.$eval('body > div > table > tbody > tr:nth-child(9) > td > table > tbody > tr:nth-child(3) > td:nth-child(5)', el => el.innerText);
    salaryArray.push(Number(salary));
    // time = time.split(' ')[6];
    // const timeAsMinutes = moment.duration(time, 'm').asMinutes();
    // minutesArray.push(timeAsMinutes);
  }
  console.log('salary -> ', salaryArray.reduce((a, b) => a + b));
